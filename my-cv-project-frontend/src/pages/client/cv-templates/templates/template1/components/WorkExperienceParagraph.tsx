                    ...
                    <TimelineItem>
                        {
                            position !== "mobile" && (
                                ...
                                >
                                    <span className="text-shadow-lg/10">{renderStartEndDate(workExperience?.workStartDate, workExperience?.workEndDate, !workExperience?.workEndDate)}</span>
                                </TimelineOppositeContent>
                            )
                        }
                        <TimelineSeparator>
                        ...
                        <TimelineContent sx={{ py: '5px', px: 2 }}>
                            <p className="text-xl md:text-2xl font-bold font-kanit text-shadow-lg/10">{workExperience?.jobPositionName}</p>
                            <p className="text-base md:text-lg font-kanit text-shadow-lg/10">{workExperience?.jobDescription}</p>
                        </TimelineContent>
                    </TimelineItem>
                    {
                            ...
                            <TimelineItem key={`${workExperienceIndex}_${projectIndex}`}>
                                {
                                    position !== "mobile" && (
                                        <TimelineOppositeContent
                                            ...
                                            fontFamily={"monospace"}
                                        >
                                            <span className="text-shadow-lg/10">{renderStartEndDate(project?.projectStartDate, project?.projectEndDate, projectIndex === 0)}</span>
                                        </TimelineOppositeContent>
                                    )
                                }
                                <TimelineSeparator>
                                ...
                                <TimelineContent sx={{ py: '20px', px: 2 }}>
                                    <p key={`project_${projectIndex}`} className="text-lg md:text-xl font-bold font-kanit mb-1 text-shadow-lg/10">
                                        {project?.projectName}
                                    </p>
                                    {
        ...
        <>
            <div className="flex flex-col items-center justify-center pt-8 bg-gray-600">
                <div className="w-full flex flex-col font-bold items-center justify-center pt-0 md:pt-4">
                    ...
                    <hr className="w-9/10 sm:w-4/10 md:w-2/3 border-0.5 md:border-1 border-white mt-4 mb-2" />
                </div>
                {/* Desktop */}
